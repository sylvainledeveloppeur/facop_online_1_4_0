<?php 	// On inclut l'entete'.
  ...
  require_once("source/section/doctype.php");//insertion du dotype html5 - style - vieport - compression de page
...
//insertion de class personalisé selon chaque page 
     require_once('source/class/store.class.php');
     require_once('source/class/blog.class.php');
...
$TheBS=!empty($_SERVER['HTTP_USER_AGENT']) ? $defaultClass_OB-> DetectBS($tams,$defaultClass_OB,$_SERVER['HTTP_USER_AGENT']) : $defaultClass_OB->DetectBS($tams,$defaultClass_OB,0);
		
//os **************************************************************************

$TheOS=!empty($_SERVER['HTTP_USER_AGENT']) ? $defaultClass_OB-> DetectOS($tams,$defaultClass_OB,$_SERVER['HTTP_USER_AGENT']): $defaultClass_OB->DetectOS($tams,$defaultClass_OB,0);

//IP+ *****************************************************************

!empty($_SERVER['REMOTE_ADDR']) ? $defaultClass_OB->InserVisite($tams,$defaultClass_OB,$_SERVER['REMOTE_ADDR'],dirname($_SERVER['SERVER_PROTOCOL']) . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],$deviceType,$TheOS,$TheBS): "";
