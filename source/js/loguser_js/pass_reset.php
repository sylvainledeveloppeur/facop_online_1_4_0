...
extract($_POST);
   ...
   $token=$t;
    $autor="PIDAF";
    $url="www.pidaf.org";
     $thelink='http://pidaf.org/index.php?b=loguser.login.loguser';
             ...
             if ($pas==$pas2 )
               {
                            ...
                            $time_60min = time() - (60 * 24);
                                 ...
                                 $nbr_pseudo=$tams->prepare("SELECT COUNT(id) nbr FROM user_passforgot
                               ...
                               if ($chif_pse['nbr']==1)
                                {
                                                    ...
                                                    $reqe =$tams->exec("UPDATE user SET pass='".sha1($pas)."'  WHERE email='$m'  ");
                                                        
                                                    $reqe1 =$tams->exec("UPDATE user_passforgot SET valid=1  WHERE email='$m'  ");
                    ...
                    $reSu=@$defaultClass_OB->senMail("Mot de passe Modifié",$autor,$m,"no_reply@pidaf.org",$m,$tams,$message_html1,$message_txt1);
                                ...
                                {
                                 echo '<div class="no_form">Erreur "Le lien a expiré"</div>';	
                                }
          }
        else{
            echo '<div class="no_form">Erreur "Les 2 mot de passe sont differents" </div>';
        }                   
	}
	else{
		echo '<div class="no_form">Erreur "6 caractères minimum pour le mot de passe" </div>';
	}
}
