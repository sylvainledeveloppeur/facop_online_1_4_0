...
if(!empty($_SESSION['pseudo']) AND !empty($_POST['status']) AND $_POST['status']=="successful" )
  {
     //echo $_POST["amo"].$_POST["tra"]." Panier sauvegardeeeeeeeeeeee";
	//$data=1724169998096 HHH 782 HHH vedette HHH 59781 HHH mdietvedette0@gmail.com HHH 23facop24 HHH  HHH 10 KKK 1 KKK facop_a1 KKK Facop A1 KKK 10000 XXX  HHH 10000.0 HHH FLUTTERWAVE_MOBILE HHH FLUTTERWAVE_MOBILE_CM_XAF
	
	$custom=explode(" HHH ",$_POST['custom']);

$allPacks=$custom[7];//$allPacks=substr($custom[7], 0, -5) ;
	 ...
	 $result="";
                        ...
                        for($i=0; $i<$coun_allPacks; $i++)
                        {
                          ...
                          $inser=$req->execute(array( $custom[0], $custom[1], $custom[2], $onePack[0], $onePack[1], $onePack[2], $onePack[3], $custom[8], $custom[10], 1, $_POST['custom'], "", $BClas->phpDate() ));
							...
							//array_push($_SESSION['achat'], $onePack[2]);
                        ...
                        if ($inser)
                          {
                                      ...
                                      if(!empty($custom[5]) AND !empty($custom[6]))
                                      {
                                              ...
                                              while($outilA=$blocA->fetch())
                                                ...
                                                VALUES (?,?,?,?,?,?,?,NOW()) ');
                                                $inser=$req->execute(array($custom[1], $custom[2], $custom[4], $_SERVER["REMOTE_ADDR"],"ACHAT PACK", $custom[2].", a acheté le pack (".$buyPack."). Parrain direct:".$custom[5].". Parrain indirect:".$custom[6]." _(WEBSITE)",0));
                                      ...
                                      else
                                      {
                                                ...
                                                $req=$tams->prepare('INSERT INTO _admin_historique_user (id_hu, user, emai, ip ,type, messag,lu,dates ) 
                                                VALUES (?,?,?,?,?,?,?,NOW()) ');
                                                $inser=$req->execute(array($custom[1], $custom[2], $custom[4], $_SERVER["REMOTE_ADDR"],"ACHAT PACK", $custom[2].", a acheté le pack (".$buyPack."). Parrain direct: --- . Parrain indirect: ---  _(WEBSITE)",0));
										...
										// redirection 
										 ...
										 $BClas->redirectPg("index.php?b=user.cours.user&pay=1");
										   echo  '<div class="ok_form">Félicitation</div>';
										   $BClas->showInfo('<h2>Félicitation</h2><p>Votre paiement à reussi</span></p>');
                          }
                          else
                          {
                              $result="false";
							   echo '<div class="no_form">Erreur lors de la sauvegarde</div>';
	                         $BClas->showInfo('<h2>Ereur</h2><p>Erreur lors de la sauvegarde</span></p>');
                          }
...
{
	   echo '<div class="no_form">Erreur lors du paiement</div>';
	   $BClas->showInfo('<h2>Ereur</h2><p>Erreur lors du paiement</span></p>');
}
?>