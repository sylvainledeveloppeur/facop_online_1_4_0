	   ...
	   $nbr_pseudo=$tams->prepare("SELECT COUNT(id_pro) nbr FROM store
		...
		/*save base de donnée*/
					  ...
					  $req=$tams->prepare('INSERT INTO store (admin_pro,actif_pro,id_aut_pro,id_cat_pro,id_soucat_pro,img_pro,img2_pro,tit_pro,aut_pro,nprice_pro,oprice_pro,desc_pro,date_pro ) 
							 ...
							 $inser=$req->execute(array(0,0,$_SESSION['id'],$_POST['cat'],0,$Gimg,$Mimg,$_POST['tit'],0,$_POST['pri'],0,$_POST['des'] ));
								 ...
								 if($inser)
								{
									...
									echo '<div class="ok_form">Article ajouté</div>';
                                       ...
                                       echo '<script>
                                              document.getElementById("poptex").innerHTML="<h2>Felicitation</h2><p>Vous venez d\'ajouter un article. Verifiez-le dans votre menu <span>Mes articles</span></p> ";
