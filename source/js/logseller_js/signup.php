...
extract($_POST);
    ...
    $autor="PIDAF";
    $url="www.pidaf.org";
     $thelink='http://pidaf.org/index.php?b=logseller.login.logseller&token='.$token.'&email='.$mai.'';
     ...
     $message_html1 = '
                              ...
                              Nous avons bien reçu votre inscription <strong>(compte vendeur)</strong>. Cependant, veuillez confirmer votre 
                               ...
                               if ($chif_pse['nbr']==0)
                                {
                                    ...
                                    $req=$tams->prepare('INSERT INTO seller (ava,nam,mai,val,pas,sex,adr,tel,vil,pay,date) 
                                    VALUES (?,?,?,?,?,?,?,?,?,?,NOW()) ');

                                   $inser=$req->execute(array("seller.png",$nam,$mai,0,sha1($pas),0,0,$tel,0,$pay));
                                   ...
                                   if($inser)
                                    { 
                                       ...
                                       $req=$tams->prepare('INSERT INTO historique_user (id_hu,user, emai, ip ,type, messag,lu,dates ) 
                                                                           VALUES (?,?,?,?,?,?,?,NOW()) ');
                                       $inser=$req->execute(array(0,$nam,$mai,$_SERVER["REMOTE_ADDR"],"INSCRIPTION",$nam." Vient de créer son compte vendeur",0));
