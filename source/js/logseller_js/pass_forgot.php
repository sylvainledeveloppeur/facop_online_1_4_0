...
extract($_POST);
   ...
   $token=@$defaultClass_OB->toKen(10,'a1A');
    $autor="PIDAF";
    $url="www.pidaf.org";
     $thelink='http://pidaf.org/index.php?b=logseller.forgot.logseller&o=2&t='.$token.'&m='.$mai.'';
   ...
   if (preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#",$mai) )
	{
                                  ...
                                  //recherche du mail
                                 $nbr_pseudo=$tams->prepare("SELECT COUNT(mai) nbr FROM seller
                                 WHERE mai=:pse  " ) ;
                               ...
                               if ($chif_pse['nbr']==1)
                                {
                                    ...
                                    $req=$tams->prepare('INSERT INTO password_forgot (email,token,valid,date) 
                                    VALUES (?,?,?,NOW()) ');
                    ...
                    $reSu=@$defaultClass_OB->senMail("Mot de passe oublié",$autor,$mai,"no_reply@pidaf.org",$mai,$tams,$message_html1,$message_txt1);
                                ...
                                {
                                 echo '<div class="stop_form">Erreur "Email introuvable"</div>';	
                                }
