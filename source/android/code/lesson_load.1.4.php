...
require_once("../class/db_app.class.php");
  ...
  if ($chif_pse['nbr']!=0)
    {
        $query = $tams->query("SELECT * FROM lesson WHERE id_pack='$packid' AND actif=1 ORDER BY titre ASC  "); 
        $outils = array(); 
        while ($outil = $query->fetch()) 
            {
                  ...
                  $reqUni=$tams->prepare("SELECT * FROM video  WHERE id_les_vi=".$outil["id_les"]." AND titre='1-Introduction' AND actif_vi=1 OR id_les_vi=".$outil["id_les"]." AND titre='1-Formation' AND actif_vi=1  " ) ;
                                       $reqUni->execute();
                                       $reqUniData=$reqUni->fetch();
                                       $reqUniData["youtube_FR"]=!empty($reqUniData["youtube_FR"])? $reqUniData["youtube_FR"]:"hYt-L2g-Du4";
                                       $reqUniData["youtube_EN"]=!empty($reqUniData["youtube_EN"])? $reqUniData["youtube_EN"]:"hYt-L2g-Du4";
						
						//theme terminer			   
					$reqUniA=$tams->prepare("SELECT * FROM achat WHERE iduser_cha=$user AND  idpack_cha =$packid " ) ;
                                       $reqUniA->execute();
                                       $reqUniDataA=$reqUniA->fetch();
                                       $termi=str_contains(trim($reqUniDataA['theme_cha']), $outil["id_les"]) ? "true":"false";	
                    ...
                    array_push($outils, array(
                      ...
                      "img1_data"=> $outil["img"],
                      "img2_data"=> $termi,
                      "video1_data"=> $lang=="fr" ? $reqUniData["youtube_FR"] : $reqUniData["youtube_EN"], 
