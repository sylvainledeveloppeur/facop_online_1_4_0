...
';
    */
	...
	public  function senMail($sujet,$autor,$clien,$mailSender,$mailClien,$tams,$message_html1,$message_txt1)
    {
                ...
                $header.= "Return-Path: \"".$autor."\" <".$mailSender.">".$passage_ligne;
                $header.= "CC: \"".$autor."\" <".$mailSender.">".$passage_ligne;
                $header.= "BCC: \"".$autor."\" <".$mailSender.">".$passage_ligne;
...
// time ago _____ $BClas->time_Ago(strtotime($done['phpdate_pro']),"fr");  $time_ago = strtotime("02-10-2022"); $BClas->time_Ago($time_ago) . "\n";
	...
	// Check for weeks
		...
		if($weeks == 1) {
		      return   $lang=="fr" ? "Il y a 1 semaine " : "a week ago";
		}
		else {
		      return   $lang=="fr" ? "Il y a $weeks semaines " : "$weeks weeks ago";
		}
	}
	...
	// Check for months
		...
		if($mnths == 1) {
		      return   $lang=="fr" ? "Il y a 1 mois" : "1 month ago";
		}
		else {
		      return   $lang=="fr" ? "Il y a $mnths mois" : "$mnths months ago";
		}
	}
	...
	// Check for years
		...
		if($yrs == 1) {
		      return $lang=="fr" ? "Il y a 1 an" : "1 year ago" ;
		}
		else {
		      return $lang=="fr" ? "Il y a $yrs ans" :  "$yrs years ago" ;
		}
	}
}
...
public function phpDate()
{
   ...
   return $phpdate;
	...
	function timeEnd($totalDay,$date)//time remaining
{
	$start=$date;
    $end=time();
    $dif=$end-$start;
