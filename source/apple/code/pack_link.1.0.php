...
require_once("../class/db_app.class.php");
  ...
  $nbr_pseudo=$tams->prepare("SELECT COUNT(id_pf) nbr FROM pack_link  WHERE id_pk_pf='$packid' AND actif_pf=1  
                                  " ) ;
    $nbr_pseudo->execute();
  ...
  if ($chif_pse['nbr']!=0)
    {
        $query = $tams->query("SELECT * FROM pack_link WHERE id_pk_pf='$packid' AND actif_pf=1  "); 
        $outils = array(); 
