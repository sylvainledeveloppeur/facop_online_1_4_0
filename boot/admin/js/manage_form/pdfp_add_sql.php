			...
			if(in_array(strtolower($extension), $allowed) AND in_array(strtolower($extension2), $allowed) )
			{
			   ...
			   {*/
				$ppack=str_replace(" ","_",$nivo[1]);
				  ...
				  $Gimg=$ppack.'_'.time().'_fr.'.$extension;//nom grande img
				  $Mimg=$ppack.'_'.time().'_en.'.$extension2;//nom min img
				   
				  if(move_uploaded_file($_FILES['img']['tmp_name'], '../../../../source/pdf/pack/'.$Gimg ) AND move_uploaded_file($_FILES['img2']['tmp_name'], '../../../../source/pdf/pack/'.$Mimg ) )
				  {	
					  ...
					  if($tatu)
							{
									       ...
									       $req=$tams->prepare('INSERT INTO pack_pdf (actif_pf, id_pk_pf, pack,pack_code,titre,titreEN, link, linkEN, pg,datephp,datesql ) 
											VALUES (?,?,?,?,?, ?,?,?,?,?,NOW()) ');


									
											
											$inser=$req->execute(array(0, $nivo[0], $nivo[1], 0, $_POST['nom'], $_POST['nomen'], $Gimg,  $Mimg, $_POST['dur'],  time()));
												...
												if($inser)
												{
													...
													$Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "A ajouter, PDF (pack) ", $_POST['nom']); //notification

												}
												else
												{
												$Beclas->echoo1(0,"ERROR: contact the developer"); 
												$Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "A ajouter, PDF(pack): ERROR ", $_POST['nom']);
												}
