...
{ 
	...
	$upd=$tams->query(" UPDATE pack_link SET 
	titre=\"".$_POST['desc']."\", titreEN=\"".$_POST['descen']."\",  id_pk_pf=\"".$nivo[0]."\",  pack=\"".$nivo[1]."\", link=\"".$_POST['nom']."\", linkEN=\"".$_POST['nomen']."\"   WHERE id_pf=\"".$_POST['id']."\" ");

									if($upd)
										{
                                            $Beclas->echoo1(1,"Successfully Updated");
                                            $Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "Updated, Lien (pack)", $_POST['nom']); //notification
										}
										else
										{ 
										  $Beclas->echoo1(0,"ERROR: contact the developer"); 
										  $Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "Updated, Lien (pack): ERROR", $_POST['nom']);
										}
