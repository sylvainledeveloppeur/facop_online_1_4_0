					  ...
					  if($tatu)
							{
									if(is_file("../../../../source/video/lesson/".$_POST['desc']) AND is_file("../../../../source/video/lesson/".$_POST['descen']))
									{
									       ...
									       $req=$tams->prepare('INSERT INTO video (actif_vi, id_les_vi, lesson,lesson_code,titre,titreEN, link, linkEN, dure,img,datephp,datesql ) 
											VALUES (?,?,?,?,?, ?,?,?,?,?,?,NOW()) ');


									
											
											$inser=$req->execute(array(0, $nivo[0], $nivo[1], 0, $_POST['nom'], $_POST['nomen'], $_POST['desc'],  $_POST['descen'], $_POST['dur'], $Gimg, $Beclas->phpDate()));
