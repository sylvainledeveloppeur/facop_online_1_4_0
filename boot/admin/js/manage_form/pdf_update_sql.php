...
{ 
	...
	$upd=$tams->query(" UPDATE pdf SET 
	titre=\"".$_POST['nom']."\", titreEN=\"".$_POST['nomen']."\",  id_les_pf=\"".$nivo[0]."\",  lesson=\"".$nivo[1]."\", pg=\"".$_POST['dur']."\", link=\"".$_POST['desc']."\", linkEN=\"".$_POST['descen']."\"   WHERE id_pf=\"".$_POST['id']."\" ");

									if($upd)
										{
                                            $Beclas->echoo1(1,"Successfully Updated");
                                            $Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "Updated, PDF ", $_POST['nom']); //notification
										}
										else
										{ 
										  $Beclas->echoo1(0,"ERROR: contact the developer"); 
										  $Beclas->notify($tams, $_SESSION['id_admin'],$_SESSION['pseu'], "Updated, PDF: ERROR", $_POST['nom']);
										}
...
if(isset($_FILES['aimg']) && $_FILES['aimg']['error'] == 0) 
		{
			...
			if(in_array(strtolower($extension), $allowed))
			{
				...
				@unlink('../../../../source/pdf/lesson/'.$_POST['desc']);
				  if(move_uploaded_file($_FILES['aimg']['tmp_name'], '../../../../source/pdf/lesson/'.$_POST['desc'] ))
				  {	
...
if(isset($_FILES['aimg2']) && $_FILES['aimg2']['error'] == 0)
		{
			...
			if(in_array(strtolower($extension), $allowed))
			{
				...
				@unlink('../../../../source/pdf/lesson/'.$_POST['descen']);
				  if(move_uploaded_file($_FILES['aimg2']['tmp_name'], '../../../../source/pdf/lesson/'.$_POST['descen'] ))
				  {	
