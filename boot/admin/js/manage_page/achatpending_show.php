...
$Beclas->checkSel();
	...
	while($done=$bloc->fetch())
	{
		//$cat[$done['id']]=$done['name_fr_maqs'];
	        ...
	        $nbr_pseudo1=$tams->prepare("SELECT SUM(prixpack_cha) nbr FROM achat_pending " );
           $nbr_pseudo1->execute();
           $chif_pse1=$nbr_pseudo1->fetch();
		   $amountotal=$chif_pse1['nbr']; 
				  
			$nbr_pseudo=$tams->prepare("SELECT COUNT(id_cha) nbr FROM achat_pending " );
           $nbr_pseudo->execute();
  ...
  <tbody>
	...
	<tr>
      ...
      <td scope="col" align="center" colspan="2"><div class="coltota tex_center mar_top_40">Montant total:'.$amountotal.' XAF</div></td>
	  	   ...
	  	   ';
				...
				<tr>
			   	...
			   	$bloc=$tams->query(' SELECT * FROM achat_pending LEFT JOIN user ON achat_pending.iduser_cha=user.id  ORDER BY  id_cha DESC LIMIT ' . $premierMessageAafficher . ', ' . $nbr_ParPage);
			...
			$ava=is_file('../../../../source/img/avatar/user/'.$done['ava']) ? '../../source/img/avatar/user/'.$done['ava'] : "../../source/img/avatar/avatar.png";
				...
				<tr>
	 <td data-title="No">'.$ii.'  
	 </br> </br> '.$Beclas->onlineState($done['statu_cha'], "Payé", "Initialisé").' 
	 ...
	 </br></br> Metode paiement: </br>'.$done['methodpaie_cha'].'</td>
		  ...
		  <li><a href="#" class="linkp0" data-page="plat_upd" data-title="Edit product" data-parametre="id='.$done['id_cha'].'&type=1" ><i class="fa fa-edit" title="Edit product"></i></a></li> 
			...
			';	
			   	...
			   	echo ' <div class="after"></div>
							  ...
							  {
							  echo '<li class="linkNB '.$i.'" data-page="achatpending_show" data-title="Les achats initialisés page('.$i.')" data-para="page=achatpending_show&pg='.$i.'">
							  <a href="#">' . $i . '</a></li> ';
							  }
							  echo '</ul>'; 
