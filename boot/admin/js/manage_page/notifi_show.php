  ...
  <tbody>
      ...
      <td scope="col" align="center" colspan="2"><div class="coltota tex_center mar_top_40">---</div></td>
	  	   ...
	  	   ';
				...
				<tr>
			   	...
			   	$bloc=$tams->query(' SELECT * FROM user_notification LEFT JOIN user ON user_notification.id_user_noti=user.id  ORDER BY  id_noti DESC LIMIT ' . $premierMessageAafficher . ', ' . $nbr_ParPage);
			...
			$ava=is_file('../../../../source/img/avatar/user/'.$done['ava']) ? '../../source/img/avatar/user/'.$done['ava'] : "../../source/img/avatar/avatar.png";
				...
				echo '
				 <tr>
      <td data-title="No">'.$ii.' 
	  </br> '.$Beclas->onlineState($done['lu_noti'], "Lu", "Non lu").'</td>
      <td data-title="Preview"><img src="'.$ava.'"  alt=""/></td>
      <td data-title="Pseudo">'.$done['pseudo'].' </td>
      <td data-title="Sujet">'.$done['suj_noti'].' </td>
      <td data-title="Message"> '.$done['desc_noti'].' </td>
      <td data-title="Date">'.$done['phpdate_noti'].'
	...
	/* 	 $bloc2=$tams->query(' SELECT * FROM _marque_sto WHERE id_maqs =\''.$done['id_marque_pro'].'\' ORDER BY  id_maqs DESC ');
       ...
       echo'<br/><br/>
		...
		<ul class="supedit">
			 <li><a href="#" data-table="recus" data-champid="id_rec" data-champ="actif HHH '.$done['id_noti'].' HHH Page=Les Réçus; Elève='.$done['id_noti'].'; Reçu='.$done['id_noti'].'"  data-action="activer_sql"    data-id="'.$done['id_noti'].'" class="action0"><i class="fa fa-check" title="activate product"></i></a></li>
			
		  <li><a href="#" class="linkp0" data-page="plat_upd" data-title="Edit product" data-parametre="id='.$done['id_noti'].'&type=1" ><i class="fa fa-edit" title="Edit product"></i></a></li> 
		  
			<li class="Udelet"><i class="fa fa-remove" title="Delete product"></i></li>
			<a href="gf" data-table="user_notification" data-champid="id_noti" data-champ="../../../../source/pdf/certificat/user/'.$done['id_noti'].' HHH 1 HHH 2 HHH 3 HHH 4 HHH 5 HHH Page=Les notifications; Utilisateur='.$done['pseudo'].'; notifications='.$done['desc_noti'].'" data-action="delete_sql"  data-id="'.$done['id_noti'].'" class="Uconfirm action"><i class="fa fa-remove" title="Delete product">Confirm</i></a>
		  </ul></td>
    </tr>
			';	
					$ii++;
			   	...
			   	echo ' <div class="after"></div>
							  ...
							  {
							  echo '<li class="linkNB '.$i.'" data-page="notifi_show" data-title="Notifications utilisateurs page('.$i.')" data-para="page=notifi_show&pg='.$i.'">
							  <a href="#">' . $i . '</a></li> ';
							  }
							  echo '</ul>'; 
