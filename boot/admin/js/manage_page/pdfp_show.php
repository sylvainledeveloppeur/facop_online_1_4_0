  ...
  <tbody>
    ...
    <tr>
	  ...
	  //$_POST['pg']=2;
				  	      ...
				  	      $nbr_pseudo=$tams->prepare("SELECT COUNT(id_pf) nbr FROM pack_pdf " );
           $nbr_pseudo->execute();
		   ...
		   if ($chif_pse['nbr']!=0)
			{
					...
					$page = 1; // On se met sur la page 1 (par défaut)
							...
							<input name="aok" id="aok" type="submit" value="Ajouter PDF (pack)"  class="linkp btnUp" data-page="pdfp_add" data-title="Ajouter PDF (pack)" data-parametre="id=1&type=1" >
							   '; 
	      
			   
			   	$bloc=$tams->query(' SELECT * FROM pack_pdf  ORDER BY  id_pf DESC LIMIT ' . $premierMessageAafficher . ', ' . $nbr_ParPage);
      ...
      <td data-title="Preview"><img src="../../source/img/pdf.png"  alt=""/></td>
      <td data-title="Title">'.$done['titre'].'</td>
      <td data-title="Pack">'.$done['pack'].'</td>
      <td data-title="Page"> '.$done['pg'].' pages</td>
	...
	/* 	 $bloc2=$tams->query(' SELECT * FROM _marque_sto WHERE id_maqs =\''.$done['id_marque_pro'].'\' ORDER BY  id_maqs DESC ');
       ...
       echo'<br/>PDF :<br/>
	   <a href="../../source/pdf/pack/'.$done['link'].'" target="_blanc" class="petibtn">Télécharger  [fr]</a><br/>
	   <a href="../../source/pdf/pack/'.$done['linkEN'].'" target="_blanc" class="petibtn">Télécharger  [en]</a>
		...
		<ul class="supedit">
			 <li><a href="ddf" data-table="pack_pdf" data-champid="id_pf" data-champ="actif_pf HHH '.$done['actif_pf'].' HHH Page=PDF(pack); Nom='.$done['titre'].'; Prix='.$done['titre'].'"  data-action="activer_sql"    data-id="'.$done['id_pf'].'" class="action"><i class="fa fa-check" title="activate product"></i></a></li>
			
		  <li><a href="5d5ed" class="linkp" data-page="pdfp_update" data-title="Update PDF(pack)" data-parametre="id='.$done['id_pf'].'&type=1" ><i class="fa fa-edit" title="Edit product"></i></a></li>
		  
			<li class="Udelet"><i class="fa fa-remove" title="Delete product"></i></li>
			<a href="gf" data-table="pack_pdf" data-champid="id_pf" data-champ="../../../../source/pdf/pack/'.$done['link'].' HHH ../../../../source/pdf/pack/'.$done['linkEN'].' HHH 2 HHH 3 HHH 4 HHH 5 HHH Page=PDF(pack); Nom='.$done['titre'].'; id='.$done['id_pf'].'"  data-action="delete_sql"   data-id="'.$done['id_pf'].'" class="Uconfirm action"><i class="fa fa-remove" title="Delete product">Confirm</i></a>
		  </ul></td>
    </tr>
			';	
					$ii++;
