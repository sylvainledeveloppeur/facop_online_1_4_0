...
<table width="100%" border="1" class="utable utable_1">
  <tbody>
    ...
    <tr>
	  ...
	  $nbr_pseudo=$tams->prepare("SELECT COUNT(id_vis) nbr FROM _admin_visite WHERE online_vis>$timestamp_5min
		    " ) ;
           $nbr_pseudo->execute();
	  ...
	  echo  '<div class="ok_form tex_center mar_top_40">Total: '.$chif_pse['nbr'].'</div>';
			...
			$bloc=$tams->query('SELECT * FROM _admin_visite  ORDER BY datevist2_vis DESC ');
				...
				{
					$timestamp_5min = time() - (60 * 5); // 60 * 5 = nombre de secondes écoulées en 5 minutes
					if($done['online_vis']< $timestamp_5min)
					{}
						else
						{
				echo '
      ...
      <td data-title="Online"><mark>Online</mark></td>
